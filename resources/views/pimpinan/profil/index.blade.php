    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                ...
                <div class="px-6 pb-6">
                    <div class="flex flex-col items-center -mt-12">
                        <!-- Avatar Form (Terpisah) -->
                        <form id="avatarForm" action="{{ route(Auth::user()->role . '.profil.update') }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            
                            <!-- Hidden fields untuk lolos validasi -->
                            ...
                            <div class="relative">
                                <div class="w-24 h-24 rounded-full border-4 border-white shadow-lg overflow-hidden">
                                    ...
                                    @else
                                        <div class="w-full h-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-white font-bold text-3xl" id="avatarInitials">
                                ...
                                <!-- Upload Button -->
                                <button type="button" onclick="document.getElementById('avatarInput').click()" class="absolute bottom-0 right-0 w-8 h-8 bg-white rounded-full shadow-lg flex items-center justify-center text-blue-600 hover:bg-blue-50 transition-colors">
                        ...
                        <!-- Remove Avatar Button (jika ada avatar) -->
                        @if($user->avatar)
                        <form action="{{ route(Auth::user()->role . '.profil.update') }}" method="POST" class="mt-2">
                            @csrf
                            @method('PUT')
                            <input type="hidden" name="remove_avatar" value="1">
                            <button type="button" onclick="confirmRemoveAvatar(this.form)" class="text-xs text-red-600 hover:text-red-700 font-medium">
                                Hapus Foto
                            </button>
                        </form>
                        @endif
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                ...
                <form action="{{ route(Auth::user()->role . '.profil.update') }}" method="POST">
                    @csrf
                    @method('PUT')
    ...
    if (input.files && input.files[0]) {
        ...
        reader.onload = function(e) {
            ...
            if (preview) {
                ...
                // Ganti initials dengan image
                initials.outerHTML = '<img src="' + e.target.result + '" alt="Avatar" class="w-full h-full object-cover" id="avatarPreview">';
            }
        };
...
function confirmRemoveAvatar(form) {
    Swal.fire({
