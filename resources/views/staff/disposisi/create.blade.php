...
<div class="max-w-4xl mx-auto" x-data="{ itemType: 'arsip', needsForwarding: false, showFinalRecipient: false }">
    ...
    <!-- Forwarding Info (for Staff/Pimpinan) -->
        ...
        <div class="flex items-start">
            ...
            <div>
                ...
                <ul class="text-sm text-blue-700 space-y-1">
                    ...
                    <li>• Jika ingin mengirim ke Staff/Pimpinan lain, disposisi akan <strong>diteruskan melalui Admin</strong></li>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <form action="{{ route(Auth::user()->role . '.disposisi.store') }}" method="POST" class="p-6 space-y-6">
            @csrf
            ...
            <!-- Recipient Section -->
            @if(Auth::user()->role === 'admin')
                <!-- Admin: Direct recipient selection -->
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">
                    ...
                    <select name="to_user_id" required
                        ...
                        <option value="{{ $user->id }}">{{ $user->name }} - {{ $user->role }} ({{ $user->unit ?? '-' }})</option>
                        @endforeach
                    </select>
                </div>
            @else
                <!-- Staff/Pimpinan: Choice between direct to admin or forwarding -->
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">
                        Penerima Disposisi <span class="text-red-500">*</span>
                    </label>
                    ...
                    <div class="mb-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
                        <label class="flex items-center cursor-pointer">
                            ...
                            <span class="ml-3 text-sm font-medium text-gray-700">
                                Kirim ke Staff/Pimpinan lain (melalui Admin)
                            </span>
                        </label>
                        <p class="text-xs text-gray-500 ml-8 mt-1">
                            Jika dicentang, disposisi akan diteruskan Admin ke penerima yang Anda pilih
                        </p>
                    </div>
                    ...
                    <div x-show="showFinalRecipient" x-cloak>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">
                            Penerima Akhir <span class="text-red-500">*</span>
                        </label>
                        <select name="final_recipient_id"
                                :required="needsForwarding"
                                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500">
                            <option value="">-- Pilih Penerima Akhir --</option>
                            @php
                        ...
                        <p class="text-xs text-gray-500 mt-1">
                            Disposisi akan dikirim ke Admin terlebih dahulu, lalu Admin akan meneruskannya ke penerima ini
                        </p>
                    </div>
