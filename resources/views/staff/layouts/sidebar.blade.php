@php
    $currentRoute = Request::routeIs('staff.*') ? Request::route()->getName() : '';
    $activeRouteClass = 'active';
    $baseIconClass = 'w-5 h-5 mr-3 flex-shrink-0';
    $isActive = fn ($route) => Request::routeIs($route) ? $activeRouteClass : '';
@endphp

<div id="sidebar" class="sidebar fixed h-full p-4 flex flex-col justify-between z-10">
    <div class="flex flex-col h-full">

        <button id="sidebar-toggle" class="rounded-full sidebar-toggle-btn">
            <svg id="toggle-icon" class="w-6 h-6 text-gray-700" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path>
            </svg>
        </button>

        <div class="text-2xl font-bold text-gray-800 mb-8 border-b border-gray-100 pb-4 flex items-center sidebar-title">
            <img src="https://ui-avatars.com/api/?name=G&background=2563eb&color=fff&size=40" alt="Logo" class="w-8 h-8 mr-2 rounded-full">
            <span class="text-blue-700">GANDARIA</span>
        </div>

        <nav class="space-y-1 sidebar-menu overflow-y-auto pb-4" style="max-height: calc(100vh - 180px);">

            <p class="text-xs text-gray-400 font-semibold uppercase mb-2 sidebar-title">BERANDA</p>

            <a href="{{ route('staff.dashboard') }}" class="sidebar-link {{ $isActive('staff.dashboard') }}">
                <svg class="{{ $baseIconClass }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m0 0l-7 7m7-7V4a1 1 0 00-1-1h-1m-4 17V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v16a1 1 0 001 1h4a1 1 0 001-1z"></path></svg>
                <span>Dashboard</span>
            </a>

            <p class="text-xs text-gray-400 font-semibold uppercase pt-4 pb-2 sidebar-title">SURAT & ARSIP</p>

            {{-- ✅ MENU BARU: Surat Masuk --}}
            <a href="{{ route('staff.surat-masuk.index') }}"
               class="sidebar-link {{ Request::routeIs('staff.surat-masuk.*') ? 'active' : '' }}">
                <svg class="{{ $baseIconClass }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/>
                </svg>
                <span>Surat Masuk</span>
            </a>

            {{-- Disposisi --}}
            <a href="{{ route('staff.disposisi.index') }}"
               class="sidebar-link {{ Request::routeIs('staff.disposisi.*') ? 'active' : '' }}">
                <svg class="{{ $baseIconClass }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                <span>Disposisi</span>
            </a>

            {{-- Arsip Digital --}}
            <a href="{{ route('staff.arsip.index') }}"
               class="sidebar-link {{ Request::routeIs('staff.arsip.index') ? 'active' : '' }}">
                <svg class="{{ $baseIconClass }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"></path></svg>
                <span>Arsip Digital</span>
            </a>

            {{-- Arsip Favorit --}}
            <a href="{{ route('staff.arsip.favorit') }}"
               class="sidebar-link {{ Request::routeIs('staff.arsip.favorit') ? 'active' : '' }}">
                <svg class="{{ $baseIconClass }}" fill="currentColor" viewBox="0 0 20 20">
                    <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.929 8.72c-.783-.57-.381-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                </svg>
                <span>Arsip Favorit</span>
            </a>

            <p class="text-xs text-gray-400 font-semibold uppercase pt-4 pb-2 sidebar-title">MANAJEMEN</p>

            {{-- Notifikasi --}}
            <a href="{{ route('staff.notifikasi.index') }}"
               class="sidebar-link {{ Request::routeIs('staff.notifikasi.*') ? 'active' : '' }}">
                <svg class="{{ $baseIconClass }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"></path></svg>
                <span>Notifikasi</span>
            </a>

            {{-- Manajemen Aset --}}
            <a href="{{ route('staff.aset.index') }}"
               class="sidebar-link {{ Request::routeIs('staff.aset.*') ? 'active' : '' }}">
                <svg class="{{ $baseIconClass }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m-1 4h1m8-12h2M12 7h1m-1 4h1m-1 4h1m8-4h2"></path></svg>
                <span>Manajemen Aset</span>
            </a>

            {{-- Mutasi Aset --}}
            <a href="{{ route('staff.mutasi.index') }}"
               class="sidebar-link {{ Request::routeIs('staff.mutasi.*') ? 'active' : '' }}">
                <svg class="{{ $baseIconClass }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4"/></svg>
                <span>Mutasi Aset</span>
            </a>

            {{-- Pemusnahan Aset --}}
            <a href="{{ route('staff.pemusnahan.index') }}"
               class="sidebar-link {{ Request::routeIs('staff.pemusnahan.*') ? 'active' : '' }}">
                <svg class="{{ $baseIconClass }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                <span>Pemusnahan Aset</span>
            </a>

            {{-- Manajemen User --}}
            <a href="{{ route('staff.user.index') }}"
               class="sidebar-link {{ Request::routeIs('staff.user.*') ? 'active' : '' }}">
                <svg class="{{ $baseIconClass }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20v-2a3 3 0 015.644-1.857M7 20H2v-2a3 3 0 015.356-1.857M12 12a3 3 0 100-6 3 3 0 000 6zm-5.356-2.143A9.006 9.006 0 0112 3h1.344a9 9 0 110 18H12a9 9 0 01-5.356-1.857z"></path></svg>
                <span>Manajemen User</span>
            </a>

            <p class="text-xs text-gray-400 font-semibold uppercase pt-4 pb-2 sidebar-title">LAPORAN & SISTEM</p>

            {{-- Laporan --}}
            <a href="{{ route('staff.laporan.index') }}"
               class="sidebar-link {{ Request::routeIs('staff.laporan.*') ? 'active' : '' }}">
                <svg class="{{ $baseIconClass }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17v-4m3 4v-4m3 4v-4m-9-4h18M5 3h14a2 2 0 012 2v14a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2z"></path></svg>
                <span>Laporan</span>
            </a>

            {{-- Pengaturan --}}
            <a href="{{ route('staff.pengaturan.index') }}"
               class="sidebar-link {{ Request::routeIs('staff.pengaturan.*') ? 'active' : '' }}">
                <svg class="{{ $baseIconClass }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.82 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.82 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.82-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.82-3.31 2.37-2.37a1.724 1.724 0 002.572-1.065z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path></svg>
                <span>Pengaturan</span>
            </a>
        </nav>
    </div>

    <div class="p-4 border-t border-gray-100 sidebar-footer mt-auto">
        <p class="text-xs text-gray-400">GANDARIA Versi 1.1</p>
    </div>
</div>