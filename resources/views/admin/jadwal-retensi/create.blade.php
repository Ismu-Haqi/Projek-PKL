...
@section('title', 'Tambah Klasifikasi JRA')
...
<div class="p-6 max-w-2xl mx-auto">
    ...
    <div class="bg-white rounded-xl shadow p-8">
        <h1 class="text-xl font-bold text-gray-800 mb-6">📚 Tambah Klasifikasi Retensi Arsip</h1>
        ...
        <form method="POST" action="{{ route('admin.jadwal-retensi.store') }}" class="space-y-5">
            @csrf
            ...
            <div>
                ...
                <textarea name="deskripsi" rows="2"
                          class="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500">{{ old('deskripsi') }}</textarea>
            </div>

            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-1">Masa Simpan Aktif (tahun) <span class="text-red-500">*</span></label>
                    <input type="number" name="jangka_aktif_tahun" value="{{ old('jangka_aktif_tahun', 2) }}" min="0" required
                           class="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-1">Masa Simpan Inaktif (tahun) <span class="text-red-500">*</span></label>
                    <input type="number" name="jangka_inaktif_tahun" value="{{ old('jangka_inaktif_tahun', 3) }}" min="0" required
                           class="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500">
                </div>
            </div>
