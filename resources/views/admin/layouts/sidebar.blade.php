    ...
    <div class="flex flex-col h-full">
        ...
        <nav class="space-y-1 sidebar-menu overflow-y-auto pb-4" style="max-height: calc(100vh - 180px);">
            ...
            <a href="{{ route(Auth::user()->role . '.dashboard') }}" 
               class="sidebar-link {{ $isActive(Auth::user()->role . '.dashboard') }}">
            ...
            <a href="{{ route(Auth::user()->role . '.retensi.index') }}" 
               class="sidebar-link {{ Request::routeIs(Auth::user()->role . '.retensi.*') ? 'active' : '' }}">
            ...
            {{-- ARSIP FAVORIT --}}
            <a href="{{ route(Auth::user()->role . '.arsip.favorit') }}" 
               class="sidebar-link {{ Request::routeIs(Auth::user()->role . '.arsip.favorit') ? 'active' : '' }}">
            ...
            {{-- DISPOSISI --}}
            <a href="{{ route(Auth::user()->role . '.disposisi.index') }}" 
               class="sidebar-link {{ Request::routeIs(Auth::user()->role . '.disposisi.*') ? 'active' : '' }}">
                <svg class="{{ $baseIconClass }}" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                <span>Disposisi</span>
            </a>
            ...
            <a href="{{ route(Auth::user()->role . '.notifikasi.index') }}" 
               class="sidebar-link {{ Request::routeIs(Auth::user()->role . '.notifikasi.*') ? 'active' : '' }}">
            ...
            <a href="{{ route('admin.galeri.index') }}"
               class="sidebar-link {{ Request::routeIs('admin.galeri.*') ? 'active' : '' }}">
                <svg class="{{ $baseIconClass }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
            ...
            <a href="{{ route(Auth::user()->role . '.user.index') }}" 
               class="sidebar-link {{ Request::routeIs(Auth::user()->role . '.user.*') ? 'active' : '' }}">
            ...
            <a href="{{ route(Auth::user()->role . '.laporan.index') }}" 
               class="sidebar-link {{ Request::routeIs(Auth::user()->role . '.laporan.index') ? 'active' : '' }}">
            ...
            {{-- Pengaturan --}}
            <a href="{{ route(Auth::user()->role . '.pengaturan.index') }}" 
               class="sidebar-link {{ Request::routeIs(Auth::user()->role . '.pengaturan.*') ? 'active' : '' }}">
