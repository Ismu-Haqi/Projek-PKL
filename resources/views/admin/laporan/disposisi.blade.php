...
<div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div class="flex items-center">
            ...
            <div>
                ...
                <p class="text-gray-600 mt-1">Tracking disposisi arsip & aset berdasarkan periode</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <form method="GET" action="{{ route(Auth::user()->role . '.laporan.disposisi') }}">
            
            <!-- Period Toggle Buttons -->
            <div class="flex flex-wrap gap-2 mb-4 pb-4 border-b">
                <button type="submit" name="period" value="1month" 
                    class="px-4 py-2 rounded-lg font-medium transition-all {{ request('period', '1month') === '1month' ? 'bg-purple-500 text-white shadow-lg' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }}">
                    📅 1 Bulan
                </button>
                <button type="submit" name="period" value="3months" 
                    class="px-4 py-2 rounded-lg font-medium transition-all {{ request('period') === '3months' ? 'bg-purple-500 text-white shadow-lg' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }}">
                    📅 3 Bulan
                </button>
                <button type="submit" name="period" value="6months" 
                    class="px-4 py-2 rounded-lg font-medium transition-all {{ request('period') === '6months' ? 'bg-purple-500 text-white shadow-lg' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }}">
                    📅 6 Bulan
                </button>
                <button type="submit" name="period" value="1year" 
                    class="px-4 py-2 rounded-lg font-medium transition-all {{ request('period') === '1year' ? 'bg-purple-500 text-white shadow-lg' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }}">
                    📅 1 Tahun
                </button>
                <button type="button" onclick="toggleCustomDate()" 
                    class="px-4 py-2 rounded-lg font-medium transition-all {{ request('period') === 'custom' ? 'bg-purple-500 text-white shadow-lg' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }}">
                    🗓️ Custom
                </button>
            </div>
            ...
            <div id="customDateRange" class="mb-4 pb-4 border-b {{ request('period') === 'custom' ? '' : 'hidden' }}">
                ...
                <button type="submit" class="mt-4 px-6 py-2.5 bg-purple-500 text-white rounded-lg hover:bg-purple-600 transition-all shadow-lg font-medium">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
                    ...
                    <select name="item_type" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500">
                        ...
                        <option value="arsip" {{ request('item_type') == 'arsip' ? 'selected' : '' }}>📁 Arsip</option>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div class="bg-gradient-to-br from-yellow-50 to-orange-50 rounded-xl border border-yellow-200 p-6">
            ...
            <div class="text-xs text-gray-500 mt-2">Menunggu proses</div>
        </div>
        ...
        <div class="bg-gradient-to-br from-green-50 to-teal-50 rounded-xl border border-green-200 p-6">
            ...
            <div class="text-xs text-gray-500 mt-2">Sudah dikerjakan</div>
        </div>

        <div class="bg-gradient-to-br from-purple-50 to-pink-50 rounded-xl border border-purple-200 p-6">
            <div class="text-sm text-purple-600 font-medium mb-1">Disposisi Aset</div>
            <div class="text-3xl font-bold text-purple-700">{{ $stats['aset'] }}</div>
            <div class="text-xs text-gray-500 mt-2">Item aset</div>
        </div>
    </div>
            ...
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-6 py-4">
                            @if($disposition->disposable_type === 'App\\Models\\Archive')
                        ...
                        <td class="px-6 py-4 text-sm">
                            @if($disposition->deadline)
                                ...
                                @if($disposition->isOverdue())
                                    <p class="text-xs text-red-600 mt-1">⚠️ Terlambat</p>
                                @endif
                            @else
